...
class UserHandler(sockjs.tornado.SockJSConnection):
    def open(self):
        print 'new button connection'
        self.write_message("Hello World")

    def on_message(self, message):
        print 'button message received %s' % message

        global weioPipe
        weioPipe.stdin.write("STOP")

    def on_close(self):
      print 'button connection closed'
...
class TestHandler(sockjs.tornado.SockJSConnection):
    ...
    def on_message(self, message):
        ...
        if (message == "START"):
            ...
            global weioPipe
            weioPipe = subprocess.Popen(['python', '-u', processName], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
