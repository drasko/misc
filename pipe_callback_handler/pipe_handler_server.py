...
class WeioConnection(sockjs.tornado.SockJSConnection):
    def on_message(self, msg):
        ...
        #callback = functools.partial(self.on_subprocess_result)
        io_loop.add_handler(p.stdout.fileno(), self.on_subprocess_result, io_loop.READ)

    #def on_subprocess_result(self):
    def on_subprocess_result(self, fd, events):
        ...
        if self.pipe.poll() is not None :
            ...
            tornado.ioloop.IOLoop.instance().remove_handler(self.pipe.stdout.fileno())
            return
